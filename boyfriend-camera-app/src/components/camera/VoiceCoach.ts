...
const FACE_TIPS = {
  ...
  // 新增：水族馆场景提示
  AQUARIUM_SCENE: '水族馆蓝色光影超梦幻！靠近玻璃站，鱼群当背景～',
  // 新增：宠物入镜提示
  PET_IN_FRAME: '毛孩子入镜啦！让它们也看镜头～',
  // 新增：逆光补光提示
  FILL_LIGHT_HINT: '脸有点暗了，打开闪光灯补补光～',
  // 新增：构图黄金时机
  GOLDEN_COMPOSITION: '构图完美！就是现在，按下去！',
...
class VoiceCoach {
  ...
  async speakCategoryTip(category: string): Promise<void> {
    const tips: Record<string, string[]> = {
      ...
      '水族馆': [
        '水族馆蓝色光影超梦幻！靠近玻璃站，鱼群当背景～',
        '水族馆里光线偏暗，打开闪光灯补光让脸亮起来～',
        '蓝色水光打在脸上很梦幻！稍微侧身躲开玻璃反光～',
        '水族馆隧道里光线最均匀，找个好角度让水光打在脸上～',
      ],
      '健身': [
        '健身房镜子自拍要斜着拍，避免镜面反光～',
        '运动后皮肤红润通透！趁这个状态赶紧拍～',
  ...
  /** 人文风景场景（街拍/建筑/纪实） */
  async speakHumanitiesTip(): Promise<void> {
    const tips = [
      '人文风景重在故事感！找有光影对比的角度按下快门～',
      '街头随拍背景要干净简洁，让人物更突出～',
      '人文照讲究瞬间感，连拍几张总有一张有灵魂～',
      '拍人文时靠近一点，让被拍者成为画面主角～',
      '找个有纵深感的背景，让照片更有层次～',
      '人文风景光线要讲究，侧逆光最有质感～',
      '街道建筑做背景时，人物站在黄金分割点最好看～',
      '人文摄影要有耐心，等一个好的光线和瞬间～',
    ]
    await this.speak(pickRandom(tips), true)
  }
  ...
  /** 室内场景（咖啡馆/工作室/商场） */
  async speakIndoorSceneTip(): Promise<void> {
    const tips = [
      '室内场景找窗户边站，让自然光打在脸上～',
      '咖啡馆或工作室灯光偏暖黄，后期可以增强氛围～',
      '室内人造光偏色严重，找最近的窗户补自然光～',
      '室内拍摄让女朋友靠近光源，脸部光线会更干净～',
      '室内暗光环境要稳住手机，或者打开屏幕补光～',
      '商场或店铺灯光复杂，找一处单一光源效果最好～',
      '室内场景打开人像模式虚化杂乱的背景～',
      '室内反光严重的地方，让女朋友稍微侧身避开反射光～',
    ]
    await this.speak(pickRandom(tips), true)
  }

  /** 室内日常场景（居家/办公室/日常） */
  async speakIndoorDailyTip(): Promise<void> {
    const tips = [
      '日常居家照光线要柔和！找窗户边拍最自然～',
      '室内日常照背景要整洁，提前收拾一下画面会更干净～',
      '日常场景用自然光拍最真实，别开太多灯～',
      '居家日常照找一面干净的墙做背景就很出片～',
      '室内日常照适合柔和的侧光，让脸型更有立体感～',
      '日常场景最重要的是自然！让女朋友做自己的事抓拍最真实～',
      '室内日常光线如果偏暗，打开窗帘或靠近窗户～',
      '日常居家照可以加入生活道具，画面更有温度～',
    ]
    await this.speak(pickRandom(tips), true)
  }

  /** 户外风景场景（山川/森林/草原/旅行） */
  async speakOutdoorSceneryTip(): Promise<void> {
    const tips = [
      '户外风景人小景大！让女朋友站在画面黄金分割点～',
      '风景照要让背景干净，人物才是主角，不要被杂乱背景抢镜～',
      '户外好天气光线充足，让女朋友稍微侧光站位更有立体感～',
      '风景人像用三分法构图，天空和人物各占一半画面～',
      '户外拍摄背景有纵深感时，让人站远一点拍进更多景色～',
      '山景/湖景找开阔位置，正侧光让水面反光超美～',
      '户外旅行照让女朋友和风景互动，画面会更生动～',
      '风景照背景要干净简洁，人物要突出不被背景吞没～',
    ]
    await this.speak(pickRandom(tips), true)
  }

  /** 构图技巧场景（九宫格/三分法/引导线） */
  async speakCompositionGuideTip(): Promise<void> {
    const tips = [
      '构图技巧打开九宫格线，人物放在交叉点上最和谐～',
      '拍人像常用三分法，把人物眼睛放在画面上三分之一～',
      '引导线构图让视线自然聚集到人物身上～',
      '对称构图适合建筑或正式场合，居中站位最有气势～',
      '留白构图让画面有呼吸感，不要把画面填得太满～',
      '框架构图找门窗或树枝做前景，画面立刻有层次～',
      '对角线构图让照片更有动感和张力～',
      '构图时背景要干净，杂物入镜会破坏整体感～',
    ]
    await this.speak(pickRandom(tips), true)
